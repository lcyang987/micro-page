    ...
    <el-tabs v-model="activeName" @tab-click="tabClick">
      ...
      <router-link append :to="{ path: 'micropage', query: {
        ...
        filename: fileJson.filename
			...
			<el-table v-loading="fileJson.table.loading" :data="fileJson.table.data" border>
...
export default {
  ...
  computed: {
    ...mapGetters([
      'CustomPage'// 每个组件不同,对应数据在vuex里的state里,写在对应文件夹的fileJson里
    ]),
    fileJson () {
      return this.CustomPage// 每个组件不同,对应数据在vuex里的state里,写在对应文件夹的fileJson里
    }
  },
  methods: {
    ...mapActions([
